  ...
  return (
    ...
    >
      ...
      <div className="w-full overflow-hidden relative z-10 pt-6">
        <div className="animate-slide flex gap-12 items-center w-max px-4">
          {[...iconItems, ...iconItems].map((item, index) => (
            <div
              key={index}
              className="group flex items-center gap-3 min-w-max whitespace-nowrap"
            >
              ...
              <span className="text-white text-sm drop-shadow-sm transform transition-all duration-300 ease-in-out group-hover:-translate-y-1 group-hover:opacity-100">
                {item.text}
              </span>
            </div>
          ))}
